  ...
  struct
    ...
    *)                                                       
    ...
    let set_result res = (st, i, o, Some res) in
      ...
      | Sexp (name, subexps) ->
         let (st', i', o', subvalues) = eval_list env conf subexps
         in  (st', i', o', Some (Value.Sexp (name, subvalues)))
  ...
  struct
    ...
    module Pattern =
      struct
        ...
        let vars p =
        (* prev definition doesn't compile, use one from video *)
    ...
    *)
    ...
    let rec eval env (state, input, output, (res : Value.t option) as config) k program =
      let eval_expr_now expr = Expr.eval env config expr in
      let set_var var value = State.update var value state in
      let prepend_k stmt kont = match kont with
        | Skip -> stmt
        | _ -> Seq (stmt, kont) in
      match program with
      | Skip -> (match k with
      ...
      | Seq (prog1, prog2) ->         
         eval env config (prepend_k prog2 k) prog1
      | If (cond, positive, negative) ->
         let (_, _, _, Some (Value.Int res) as after_cond_eval) = eval_expr_now cond in
         if (res !=0)
         then eval env after_cond_eval k positive
         else eval env after_cond_eval k negative
      | (While (cond, body) as loop) ->
         let (_, _, _, Some (Value.Int res) as after_cond_eval) = eval_expr_now cond in
         if (res!=0)
         then eval env after_cond_eval (prepend_k loop k) body
         else eval env after_cond_eval Skip k
      | (Repeat (body, cond) as loop) ->
         ...
         if res == 0
         then 
           eval env after_cond_eval (prepend_k loop k) Skip
         else eval env after_cond_eval Skip k
      | Case (expr, variants) ->
